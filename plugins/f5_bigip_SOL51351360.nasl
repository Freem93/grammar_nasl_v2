...
if (description)
{
  ...
  script_set_attribute(
    ...
    value:
"Specific websocket traffic patterns may cause a disruption of service
for virtual servers configured to use the websocket profile.
(CVE-2016-9253) Note : Virtual servers configured to use the HTTP
profile (and no websocket profile) that process websocket traffic are
not vulnerable to this issue. This issue only affects the BIG-IP data
plane; there is no control plane exposure to this vulnerability."
  );
  script_set_attribute(
    attribute:"see_also",
